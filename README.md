...
本プロジェクトでは、利用者情報・ケアプラン・日々の介護記録・モニタリング記録・書類ステータスをCSVで読み込み、書類・記録の未作成・未確認・未承認・期限切れ・記録不一致・レ点不一致候補・特記事項不足などを検知します。  
さらに、必要に応じてAI下書き候補を生成し、職員が確認・修正したうえで「職員確認済み」として保存する業務フローを検証しています。

## **本MVPの見どころ**

* 4分野15書類・記録カタログを対象に、運営指導前の未対応を可視化できます
* AIは記録を確定せず、職員確認と監査ログを必須にしています
* 職員入力を勝手に削除せず、要確認項目として保持します
* 確認履歴・監査ログを残せます
* R011デモで、サービス提供記録の不足レ点、不一致候補、特記事項補填をBefore/Afterで確認できます
* 確認済み記録の全体図表示、修正版保存、修正履歴の追記まで行えます

## **開発動機**

私は訪問介護事業所で勤務する中で、運営指導前になると、現場職員だけでなく、事務員・サービス提供責任者・管理者までが数か月間にわたり書類確認に追われる状況を経験しました。

通常業務と並行して、訪問介護計画書、サービス提供記録、モニタリング記録、契約・同意書類、職員関連書類などを確認する必要があり、確認作業が属人的になりやすいことに課題を感じました。

また、介護報酬はサービス提供だけでなく、必要書類が適切に整備・保管されていることも算定要件となるため、書類不備は運営指導での指摘だけでなく、状況によっては介護報酬の返還や行政指導につながる可能性があります。

この経験から、書類確認業務を標準化・可視化し、AIを補助的に活用しながら、最終判断は職員が行う業務フローを検証したいと考え、本MVPを開発しました。
...
* 訪問介護計画書と日々のサービス提供記録の不一致
* サービス提供記録のレ点漏れ、特記事項不足
* 記録の確認・修正・承認状況を一覧で把握しづらい
* 確認待ち・修正待ちの記録が埋もれやすい
* 運営指導前の確認作業が属人化しやすい
...
本MVPは実証前のプロトタイプであり、以下は現時点での想定効果および目標値です。本値は実証による測定値ではなく、MVPで自動化・可視化する確認作業の範囲をもとに設定した目標値です。

* 運営指導前の確認作業における見落としの低減
* 書類・記録確認業務の属人化軽減
* 不足記録や未承認書類の早期発見
* 職員確認履歴・監査ログによる説明責任の向上
* 書類不備による介護報酬返還リスクの低減に貢献
* 運営指導前のセルフチェック時間を30〜50%削減することを目標とする
...
* **AI下書き候補の生成**：特記事項・月間モニタリングのAI下書き候補を、既存の記録項目（特記事項・本人発言・観察事項・数値情報・今後の対応など）の範囲内でのみ生成します。存在しない事実や医学的判断は追加しません
...
いずれの画面でも、AIやルールベース判定は記録・書類・レ点チェックを自動確定しません。下書き候補・不一致検知を提示するのみで、最終確定は必ず職員が確認・修正・保存します。

## **デモ用サービス提供記録 R011**

R011は「身体介護サービスの記録に生活援助のレ点（清掃）が混ざり、かつ訪問介護計画書で必要とされる水分補給の促し・体調観察が不足している」ケースを想定したデモです。「書類アラート一覧」の最上部に「★ デモ用：サービス提供記録AI補填体験」として常に表示されます。

書類アラート一覧からR011を開くと、Before画面で元記録を確認し、AI補填プレビューで不足レ点・特記事項のAI下書き候補・要確認項目を確認できます。不一致候補がある場合は職員確認済みにする前に最終警告が表示され、職員が確認したうえで保存します。職員確認済みにすると、確認履歴・監査ログに保存され、概要ダッシュボードの書類・記録単位カウントにも反映されます。保存後は「確認履歴・監査ログ」から全体図の確認・修正版としての保存も行えます。
...
本MVPでは、利用者情報・ケアプラン・日々の記録・モニタリング記録・書類ステータスをCSVごとに分離し、`user_id`・`record_id`・`document_id`などのキーで突合できる設計にしています。特定の1ファイルにすべての情報を詰め込むのではなく、リレーショナルデータベースに近い発想で、エンティティ単位にデータを分割しています。

また、入力データ（`data/`）と、判定・AI下書き候補生成の結果である出力データ（`outputs/`、GitHub公開用の固定サンプルは`sample_outputs/`）を分離し、Notebookでのロジック検証とStreamlitでの画面表示の両方から同じ入力データを参照できるようにしています。

CSVベースの構成はあくまで初期検証のためのものであり、本格運用時には、RDBや既存介護ソフトのデータ構造（利用者マスタ・計画書テーブル・日々の記録テーブルなど）へ拡張しやすい形を意識しています。
...
Google Colabで`notebook/care_compliance_copilot_mvp.ipynb`を開き、Google Driveをマウント・CSVデータを読み込んだうえで、すべてのセルを上から実行します。CSV読み込み→データ前処理→ルールベース判定→統合アラート作成→AI下書き候補作成→職員確認済み保存、という順に処理し、最後に統合アラート・AI下書き候補・確認済みステータス・計画書と書類ステータスの有効期間整合性などの最終チェックを行い、主要フローが正常に完了していることを確認します。

## **技術選定理由**

* **Python / pandas**：CSVベースの業務データを前処理・突合・集計するため
* **Google Colab / Notebook**：まずルールベース判定やAI下書き候補生成のロジックを検証するため
* **Streamlit**：短期間で業務フローを画面化し、MVPとして操作感を確認するため
* **CSV**：既存介護ソフトからのエクスポートを想定し、初期検証をしやすくするため
* **ルールベース判定**：未作成・未承認・期限切れ・記録不一致など、説明可能性が必要な判定を扱うため
* **生成AIプロンプト設計**：特記事項や月間モニタリングなど、文章作成を下書き候補として支援するため
...
本MVPでは実在個人情報を使用せず、すべてデモ用CSVで検証しています。

本番導入時には、外部LLMやクラウド環境へ送信する前に、氏名・住所・生年月日・電話番号・利用者ID・自由記述内の識別情報をマスキングする方針です。あわせて、送信ログ、データ保持期間、二次利用防止、利用者同意、事業所内規程との整合性を確認する必要があります。

AIへ送信する情報は必要最小限にとどめ、AIは正式記録を確定しない設計としています（詳細は「設計上のポイント」を参照）。
...
本MVPではCSV取り込みを前提としています。既存介護ソフトからのCSVエクスポートを入口にすることで、システムリプレイスなしに価値検証できることを意図しています。将来的には、介護ソフトからの定期CSVエクスポート、API連携、夜間バッチ処理による自動チェック、チェック結果の定期レポート化を想定します。介護ソフト本体に直接組み込むのではなく、まずは後付けの監査前チェックツールとして検証する位置づけです。

### 2. 入力データの拡張

訪問介護計画書PDFの自動読み取り（現状は`care_plans.csv`を構造化済みデータとして扱っている）、担当者会議記録・研修記録・苦情処理記録の中身データモデル化とAI下書き候補生成の実装、計画書連動レ点候補のマッチング精度向上（表記ゆれ吸収など）、同一record_id/monitoring_idの重複チェックまたは上書き保存を検討します。

### 3. 個人情報マスキング

外部LLMやクラウド環境へ送信する前に、氏名・住所・生年月日・電話番号・利用者ID・自由記述内の識別情報をマスキングする方針です（詳細は「セキュリティ・個人情報保護方針」を参照）。
...
* Notebookで検証したルールベース判定とAI下書き候補生成の流れをStreamlitアプリへ展開し、運営指導前の書類確認業務フローとして成立するか
