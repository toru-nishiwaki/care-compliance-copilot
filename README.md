...
起動後、サイドバーのメニューから以下の10画面を確認できます。

1. 使い方・業務フロー（業務フロー説明。入力CSVの直接確認はここのexpanderから）  
2. 概要ダッシュボード（アラート件数・重大度などの動的集計）  
3. 書類アラート一覧（統合アラート・利用者別集計）  
4. アラート根拠確認（アラートの元になった記録・書類へのドリルダウン）  
5. 計画書連動レ点候補（ケアプランと日々の記録のキーワード突合、ルールベース）  
6. 特記事項AI補填デモ（日々の記録の特記事項AI補填案プレビュー）  
7. 月間モニタリングAI下書き（月間モニタリング文書のAI下書きプレビュー）  
8. 確認キュー（未確認AI下書き・未承認記録・要確認アラートを横断して確認・保存）  
9. 職員確認済み保存（AI下書きを確認・修正し `outputs/` に保存）  
10. 確認履歴・監査ログ（職員確認済みの履歴一覧）

いずれの画面でも、AIは下書き・候補・補填案を提示するのみで、最終確定は必ず職員が確認・修正・保存します。
...
Colabで検証したロジックは、`app.py` としてStreamlitに移植済みです。現状のStreamlit版は10画面構成で、以下を操作できます。

* 業務フロー全体のガイド表示、入力CSVの直接確認  
* アラート件数・重大度などの動的集計ダッシュボード  
* 統合アラート・利用者別アラート集計の確認  
* アラートの元になった記録・書類へのドリルダウン確認  
* ケアプランと日々の記録をキーワードで突き合わせたレ点候補の確認（ルールベース）  
* 特記事項補填案・月間モニタリング下書きのプレビュー  
* 未確認AI下書き・未承認記録・要確認アラートを横断した確認キュー  
