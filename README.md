# Care Compliance Copilot（ケアコンプライアンスコパイロット）

訪問介護事業所の運営指導前チェックにおいて、書類不備・未確認・未承認の記録を検出し、必要に応じてAI下書き候補を生成するコンプライアンス確認支援MVPです。

本プロジェクトでは、利用者情報・ケアプラン・日々の介護記録・モニタリング記録・書類ステータスをCSVで読み込み、書類不備リスクを可視化します。
AIが記録を自動確定するのではなく、職員が確認・修正したうえで「職員確認済み」として保存する業務フローを検証しています。
